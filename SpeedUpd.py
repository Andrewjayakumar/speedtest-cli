...
class Homepage(QMainWindow):
    ...
    def toolbarcreation(self):      #creating the frames
        ...
        self.chartActD = QAction(QIcon.fromTheme('chart'), "Graph For Download Speed", self,        #creating action listener for the buttons
 ...
 def displaychartdownload(self):         #Ploting the download graph
        ...
        plt.bar(y_pos, performance, align='center', alpha=0.5)      #aligning the data in the graph
        plt.xticks(y_pos, "")
        plt.ylabel('Mbit/s')
    ...
    def createCSV(self):            #Saving and writting the data in CSV
        with open(self.myfile, 'w') as stream:
            ...
            for row in range(self.tableview.rowCount()):            #range
                rowdata = []
                for column in range(self.tableview.columnCount()):
                    item = self.tableview.item(row, column)
                    if item is not None:
                        rowdata.append(item.text())     #appending the data
                    else:
                        rowdata.append('')
